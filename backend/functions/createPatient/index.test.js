...
test('creates a patient with valid input', async () => {
  ...
  ddbMock.on(PutCommand).resolves({}); //Mock patient id is 7, which the code reads as current patient id
  ...
  expect(patient.patient_id).toBe('W007'); // Patiend ID should be returned 7 acc to mock data
